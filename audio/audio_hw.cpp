...
{
    unsigned int type;
    const unsigned int firstType = OUTPUT_DEEP_BUF;

    pthread_mutex_lock(&adev->lock_outputs);
    for (type = OUTPUT_DEEP_BUF; type < OUTPUT_TOTAL; type++) {
...
static void unlock_all_outputs(struct audio_device *adev, struct stream_out *except)
{
    ...
    /*
     * unlock order is irrelevant and we cannot determine the output_type before
     * OUTPUT_TOTAL, so just unlock in the same order
     */
    pthread_mutex_unlock(&adev->lock);
    for (type = OUTPUT_DEEP_BUF; type < OUTPUT_TOTAL; type++) {
        struct stream_out *out = adev->outputs[type];
